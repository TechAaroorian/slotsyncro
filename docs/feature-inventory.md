...
**Last reviewed against repository:** 2026-08-25
...
| Google OAuth login | Proposed | Keep identity scopes separate from Google Calendar permissions. |
...
| Capability | Status | Notes |
| --- | --- | --- |
| Guest confirmation email | Current | Rendered with React Email and sent with Resend. |
| ICS attachment | Current | Generated with a stable UID stored on the booking. |
| Guest-timezone email formatting | Current | Confirmation date and time use the guest timezone. |
| Separate booking and email outcomes | Current | Email failure is reported without changing successful booking state. |
